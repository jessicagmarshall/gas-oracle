...
const GasOracleFactory = artifacts.require('GasOracleFactory')
...
contract('Oracle', function (accounts) {
    ...
    beforeEach(async () => {
        ...
        gasOracleFactory = await GasOracleFactory.deployed()
    ...
    it('should test gas oracle', async () => {
        ...
        assert.equal(await gasOracle.owner(), accounts[owner1])
        await gasOracle.replaceOwner(accounts[owner2], {from: accounts[owner1]})
        assert.equal(await gasOracle.owner(), accounts[owner2])

        // Set outcome
        await utils.assertRejects(gasOracle.setOutcome(0, {from: accounts[owner1]}), "owner1 is not the gas oracle owner")
        assert.equal(await gasOracle.isOutcomeSet(), false)

        await gasOracle.setOutcome(1, {from: accounts[owner2]})
        assert.equal(await gasOracle.isOutcomeSet(), true)
        assert.equal(await gasOracle.getOutcome(), 1)
        assert.equal(await gasOracle.ipfsHash(), ipfsBytes)
    })
